...
from app.prompts.trader_value_prompt import (
    VALUE_TRADER_PROMPT,
...
def company_profile_tool(
    wrapper: RunContextWrapper[ValueTraderContext],
    ticker: str,
) -> str:
    """
...
def build_value_trader_agent() -> Agent:
    return Agent(
        name="Value Trader",
        model=settings.VALUE_TRADER_MODEL,
        instructions=VALUE_TRADER_PROMPT,
        tools=[company_profile_tool],
    )


def build_value_sector_review_agent() -> Agent:
    return Agent(
        name="Value Trader Sector Reviewer",
        model=settings.VALUE_TRADER_MODEL,
        instructions=VALUE_TRADER_SECTOR_REVIEW_PROMPT,
    )


def extract_json_from_text(text: str) -> dict[str, Any]:
    return extract_json_dict(text, "Value Trader")
...
async def review_value_sectors(research_data: dict[str, Any]) -> dict[str, Any]:
    agent = build_value_sector_review_agent()
    payload = {
    ...
    final_names = _normalize_sector_names(
        [sector.get("name", "") for sector in research_data.get("final_sectors", [])]
    )
    candidate_names = _normalize_sector_names(
        [sector.get("name", "") for sector in research_data.get("candidate_sectors", [])]
    )
    sector_lookup = {
...
def build_value_trader_input(
    ...
    if analysis_mode == "single_stock":
        ...
        payload["task"] = (
            ...
            "Return exactly one selected stock entry for that ticker only, and rate it BUY, WATCH, or SELL from a value and quality perspective. "
        ...
        payload["task"] = (
            ...
            "identify 2 to 6 potentially investable global leaders or U.S.-listed stocks/ADRs from a value and quality perspective, with fewer only if evidence is weak and never more than 9 total. "
            "You must stay within the approved sectors only. "
            "Treat this as an investment underwriting task, not an information gathering task. "
            "Use the latest filings, filing analysis, IR/transcript links, structured fundamentals, market snapshot, and recent catalysts from the evidence pack as your primary basis. "
            "Global leaders and U.S.-listed ADRs are allowed when they are cleaner expressions of the theme than a U.S. domestic alternative. "
            "Avoid defaulting to only the most obvious theme leader when a cleaner value/quality or less crowded second-order beneficiary exists."
        )
...
async def run_value_trader(
    ...
    agent = build_value_trader_agent()
    sector_review = await review_value_sectors(research_data)
    user_input, data_availability, company_lookup = build_value_trader_input(
        research_data,
    ...
    data = {
        ...
        "agent_name": "trader_value",
        "model": settings.VALUE_TRADER_MODEL,
        "user_input": user_input,
        "raw_output": raw_output,
        "decision": parsed.get("decision", ""),
        "style": parsed.get("style", "value_quality"),
