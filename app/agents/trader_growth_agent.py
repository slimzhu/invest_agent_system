...
from app.prompts.trader_growth_prompt import (
...
def company_profile_tool(
    wrapper: RunContextWrapper[GrowthTraderContext],
    ticker: str,
) -> str:
    """
...
def build_growth_trader_agent() -> Agent:
    return Agent(
        name="Growth Trader",
        model=settings.GROWTH_TRADER_MODEL,
        instructions=GROWTH_TRADER_PROMPT,
        tools=[company_profile_tool],
    )


def build_growth_sector_review_agent() -> Agent:
    return Agent(
        name="Growth Trader Sector Reviewer",
        model=settings.GROWTH_TRADER_MODEL,
        instructions=GROWTH_TRADER_SECTOR_REVIEW_PROMPT,
    )


def extract_json_from_text(text: str) -> dict[str, Any]:
    return extract_json_dict(text, "Growth Trader")
...
async def review_growth_sectors(research_data: dict[str, Any]) -> dict[str, Any]:
    agent = build_growth_sector_review_agent()
    payload = {
...
def build_growth_trader_input(
    ...
    if analysis_mode == "single_stock":
        ...
        payload["task"] = (
            ...
            "Return exactly one selected stock entry for that ticker only, and rate it BUY, WATCH, or SELL from a growth and innovation perspective. "
        ...
        payload["task"] = (
            ...
            "identify 2 to 6 potentially investable global leaders or U.S.-listed stocks/ADRs from a growth and innovation perspective, with fewer only if evidence is weak and never more than 9 total. "
            "You must stay within the approved sectors only. "
            "Treat this as an investment underwriting task, not an information gathering task. "
            "Use the latest filings, filing analysis, IR/transcript links, structured fundamentals, market snapshot, and recent catalysts from the evidence pack as your primary basis. "
            "Global leaders and U.S.-listed ADRs are allowed when they are better growth expressions of the theme than a purely domestic alternative. "
            "Explicitly consider smaller-cap, mid-cap, challenger, or emerging winners if their growth slope and upside runway are better than the dominant incumbent. "
            "At least one selected stock should be a less-consensus challenger, emerging winner, or earlier-stage enabler rather than a mature incumbent."
        )
...
async def run_growth_trader(
    ...
    agent = build_growth_trader_agent()
    sector_review = await review_growth_sectors(research_data)
    user_input, data_availability, company_lookup = build_growth_trader_input(
        research_data,
    ...
    data = {
        ...
        "agent_name": "trader_growth",
        "model": settings.GROWTH_TRADER_MODEL,
        "user_input": user_input,
        "raw_output": raw_output,
        "decision": parsed.get("decision", ""),
        "style": parsed.get("style", "growth"),
